...
if __name__ == '__main__':
    #Construcao dos caminhos dos arquivos e criacao das variaveis primordiais do modelo
    ...
    classes_alvo = [pasta for pasta in os.listdir(caminho_dataset) if os.path.isdir(os.path.join(caminho_dataset, pasta)) and pasta != "_background_noise_"]
    ...
    #Escolha do dataset para treino e teste, utilizando a versão 1 ou 2 do dataset, que aplica a transformada de Morlet e 
    # compressão dos dados em 256. Threads usadas pro tratamentos dos dados por padrao escolhido em 6.
    ...
    dataloader_treino = DataLoader(dataset_treino, batch_size=128, shuffle=True,num_workers=6)
    ...
    dataloader_teste = DataLoader(dataset_teste, batch_size=128, shuffle=False,num_workers=6)
    #Selecionar a placa de video para rodar o treinamento de forma eficiente
    ...
    #Carregamento do modelo a partir de um ponto pre treinado por motivos de teste ou aprimoramento
    ...
    if(os.path.exists(checkpoint_path)):
        ...
        erro_treino = checkpoint.get('erro_treino',[])
        acc_teste = checkpoint.get('acc_teste',[])
    else:
        start = 0
        erro_treino = []
        acc_teste = []
    ...
    #LOOP de treinamento do modelo gerenciado pelo num_age que define quantas vezes o modelo sera treinado pelo dataset de treino
    #e para cada iteracao do treino faz um teste sobre o modelo para verificar a constancia do aprendizado e possiveis erros
    for epoch in range(start,num_age):
        ...
        total = 0
        with t.no_grad():
            for audios, labels in tqdm(dataloader_teste,desc="Testing"):
                audios = audios.to(device)
                labels = labels.to(device)
        ...
        checkpoint ={'epoch' : epoch,'model_state' : modelo.state_dict(),'optim_state' : optimizer.state_dict(),
                     'erro_treino' : erro_treino,'acc_teste' : acc_teste}
    ...
    #Plot do grafico de erro medio do modelo e de acuracia pelo tempo de epocas treinadas
    plt.figure(figsize=(12, 5))

    plt.subplot(1, 2, 1)
    plt.plot(range(1, len(erro_treino) + 1), erro_treino, marker='o', color='red', label='Loss de Treino')
    ...
    plt.subplot(1, 2, 2)
    plt.plot(range(1, len(acc_teste) + 1), acc_teste, marker='o', color='blue', label='Acurácia de Teste')
